...
Source is hosted on GitHub; work items are tracked in Azure Boards. Use the `AB#<id>` prefix — Azure Boards' GitHub app recognizes it and links the commit/PR to the work item. Plain `AB#<id>` would be auto-linked to a GitHub issue instead, so avoid it.

$ARGUMENTS can be:
- A commit message (e.g. `fix the date picker`) — uses ID from `.claude/current-workitem`
- An ID + message (e.g. `AB#42 fix the date picker`) — uses provided ID

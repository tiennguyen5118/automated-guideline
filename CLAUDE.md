...
- Environment variables validated at startup via Zod in `lib/env.ts`. Local config lives in `app/.env` (copy from `.env.example`). No `process.env.X` reads outside `lib/env.ts`.
